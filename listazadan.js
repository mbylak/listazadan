    ...
    >
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        `}>
          ...
          <div className="p-4 md:p-5 pb-2 md:pb-3 flex justify-between items-end flex-shrink-0">
            ...
            {filteredTasks.length === 0 ? (
              ...
              filteredTasks.map((task, idx) => {
                ...
                return (
                  <div 
                    key={task.id}
                    className={`group relative flex items-start p-3 rounded-xl border transition-all cursor-pointer shadow-sm
                      ${selectedTaskId === task.id ? activeCardClasses : cardClasses}
                      ${task.completed ? 'opacity-50' : ''}
                    `}
                    onClick={() => handleSelectTask(task.id)}
                  >
                    {/* Checkbox (aligned top for multi-line titles) */}
                    <button 
                      className={`mr-3 mt-0.5 p-1 rounded-md transition-colors flex-shrink-0 ${task.completed ? 'text-[#0078D4]' : isDark ? 'text-gray-400 hover:bg-[#3D3D3D]' : 'text-gray-400 hover:bg-gray-200'}`}
                      ...
                      <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-2">
                        ...
                        {(task.durationMs > 0 || task.isRunning) && (
                          <span className={`text-[11px] font-mono flex items-center gap-1 ${task.isRunning ? 'text-red-500 font-bold' : 'opacity-60'}`}>
                            ...
                            {((task.durationMs + (task.isRunning ? Date.now() - task.startTime : 0)) / 60000).toFixed(0)}m
                          </span>
                        )}
                        ...
                        {/* Ikona Opisu */}
                        {task.description && (
                          <span className="text-[11px] opacity-40 flex items-center gap-1" title="Zadanie zawiera notatkę">
                            <AlignLeft size={12} />
                          </span>
                        )}
                      </div>
                    ...
                    {/* Ordering arrows (visible on hover/mobile) */}
                    <div className="flex flex-col ml-1 absolute right-2 top-2 bg-inherit md:opacity-0 md:group-hover:opacity-100 transition-opacity">
          ...
          {selectedTask ? (
            <div className="max-w-3xl mx-auto p-5 md:p-8 pb-32">
                  ...
                  >
                    ...
                    {(selectedTask.durationMs > 0 || selectedTask.isRunning) && (
                      <span className="text-sm font-mono opacity-80 mt-1 flex items-center gap-2">
                         Czas pracy: {((selectedTask.durationMs + (selectedTask.isRunning ? Date.now() - selectedTask.startTime : 0)) / 60000).toFixed(1)} min
                      </span>
                    )}
                  </button>
                </div>
              ...
              <div className="mb-8">
                ...
                <div className={`rounded-2xl border overflow-hidden ${isDark ? 'border-[#3D3D3D] bg-[#2D2D2D]' : 'border-gray-200 bg-white shadow-sm'}`}>
                  {selectedTask.subtasks.map(sub => (
                    <div key={sub.id} className={`flex items-start gap-3 group p-3 border-b last:border-0 ${isDark ? 'border-[#3D3D3D]' : 'border-gray-100'} ${sub.completed ? 'opacity-60 bg-black/5 dark:bg-white/5' : ''}`}>
                      <button onClick={() => toggleSubtaskComplete(selectedTask.id, sub.id)} className="p-1 mt-0.5">
                        {sub.completed ? <CheckSquare size={20} className="text-[#0078D4]" /> : <Square size={20} className="text-gray-400" />}
                      </button>
                      {/* Subtask Title wraps naturally */}
                      <span className={`text-[15px] flex-1 break-words leading-relaxed ${sub.completed ? 'line-through text-gray-500' : ''}`}>{sub.title}</span>
                      <button 
                        className="text-gray-400 hover:text-red-500 p-2 rounded-lg hover:bg-black/5 dark:hover:bg-white/5 transition-colors mt-0.5"
